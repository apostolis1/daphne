...
TEMPLATE_TEST_CASE("ExtractRow - DenseMatrix double naive", TAG_KERNELS,
                   uint32_t) { // NOLINT(cert-err58-cpp)
    auto argMatrix = genGivenVals<DenseMatrix<double>>(
        4, {
    ...
    extractRow<DenseMatrix<double>, DenseMatrix<double>, uint32_t>(
        resMatrix, argMatrix, selMatrix, nullptr);
    CHECK(*resMatrix == *argMatrix);
    DataObjectFactory::destroy(argMatrix);
    DataObjectFactory::destroy(resMatrix);
}

TEMPLATE_TEST_CASE("ExtractRow - DenseMatrix double expanding", TAG_KERNELS,
                   uint32_t) { // NOLINT(cert-err58-cpp)
    auto argMatrix = genGivenVals<DenseMatrix<double>>(
        4, {
               1.0, 10.0, 3.0, 7.0, 7.0, 7.0, 17.0, 1.0, 2.0, 3.0, 7.0, 7.0,
    ...
    auto expMatrix = genGivenVals<DenseMatrix<double>>(
        8, {
               1.0, 10.0, 3.0, 7.0, 7.0, 7.0, 17.0, 1.0, 2.0, 3.0, 7.0, 7.0,
               7.0, 7.0,  1.0, 2.0, 3.0, 7.0, 7.0,  7.0, 7.0, 1.0, 2.0, 3.0,
               1.0, 10.0, 3.0, 7.0, 7.0, 7.0, 17.0, 1.0, 2.0, 3.0, 7.0, 7.0,
...
TEMPLATE_TEST_CASE("ExtractRow - DenseMatrix int unordered", TAG_KERNELS,
                   uint32_t) { // NOLINT(cert-err58-cpp)
    auto argMatrix = genGivenVals<DenseMatrix<int64_t>>(
        4, {
    ...
    auto *expMatrix = genGivenVals<DenseMatrix<int64_t>>(
        4, {7, 7,  1, 2, 3, 7, 7,  7, 7, 1, 2, 3,
            1, 10, 3, 7, 7, 7, 17, 1, 2, 3, 7, 7});

    extractRow<DenseMatrix<int64_t>, DenseMatrix<int64_t>, uint32_t>(
        resMatrix, argMatrix, selMatrix, nullptr);
    CHECK(*resMatrix == *expMatrix);
    DataObjectFactory::destroy(argMatrix);
    DataObjectFactory::destroy(resMatrix);
...
TEMPLATE_TEST_CASE(
    "ExtractRow - DenseMatrix int repeated with initialized resMatrix",
    TAG_KERNELS, uint32_t) { // NOLINT(cert-err58-cpp)
    auto argMatrix = genGivenVals<DenseMatrix<int64_t>>(
        4, {
               1, 10, 3, 7, 7, 7, 17, 1, 2, 3, 7, 7,
               7, 7,  1, 2, 3, 7, 7,  7, 7, 1, 2, 3,
    ...
    auto *resMatrix = genGivenVals<DenseMatrix<int64_t>>(
        4, {7, 0,  0, 2,  3,  7, 7,  7, 7, 1, 2, 3,
            1, 10, 3, -7, -7, 7, 17, 1, 2, 3, 7, 7});
    auto *expMatrix = genGivenVals<DenseMatrix<int64_t>>(
        4, {
...
TEMPLATE_TEST_CASE("ExtractRow - DenseMatrix boundary checking", TAG_KERNELS,
                   int32_t, double) {
    using VT = TestType;
