...
TEMPLATE_PRODUCT_TEST_CASE("Reshape", TAG_KERNELS, (DenseMatrix),
                           (double, uint32_t)) {
    using DT = TestType;
    ...
    SECTION("view 1") {
        ...
        const DT *view =
            DataObjectFactory::create<DT>(initial, 0, 3, 2, 4);    // 3x2
    ...
    SECTION("view 2") {
        ...
        const DT *view =
            DataObjectFactory::create<DT>(initial, 1, 2, 0, 6);    // 1x6
    ...
    SECTION("view 3") {
        const DT *initial = genGivenVals<DT>(2, vals); // 2x6
        const DT *view =
            DataObjectFactory::create<DT>(initial, 1, 2, 0, 4); // 1x4
