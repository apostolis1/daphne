...
template <class DT>
void checkDiagVector(const DT *arg, DenseMatrix<typename DT::VT> *&res,
                     DenseMatrix<typename DT::VT> *expectedMatrix) {
...
TEMPLATE_PRODUCT_TEST_CASE("DiagVector-normal", TAG_KERNELS, (DATA_TYPES),
    ...
    auto *expectedMatrix =
        genGivenVals<DenseMatrix<typename DT::VT>>(3, {3, 2, 1});
...
TEMPLATE_PRODUCT_TEST_CASE("DiagVector-mixed-diagonal", TAG_KERNELS,
    ...
    auto *expectedMatrix =
        genGivenVals<DenseMatrix<typename DT::VT>>(3, {1, 0, 1});
