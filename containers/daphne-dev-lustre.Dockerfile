...
RUN apt-get -qq -y update && apt-get -y upgrade && apt-get -y --no-install-recommends install  \
    ...
    && apt-get clean && rm -rf /var/lib/apt/lists/*
COPY --from=apostolis1/daphne-deps-lustre /usr/local/bin/ /usr/local/bin/
COPY --from=apostolis1/daphne-deps-lustre /usr/local/include/ /usr/local/include/
COPY --from=apostolis1/daphne-deps-lustre /usr/local/lib/ /usr/local/lib/
COPY --from=apostolis1/daphne-deps-lustre /usr/local/share/ /usr/local/share/
RUN ldconfig
