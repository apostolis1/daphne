...
antlrcpp::Any DaphneDSLVisitor::visitAssignStatement(
    ...
    if (numVars == 1) {
        ...
        handleAssignmentPart(loc, ctx->IDENTIFIER(0)->getText(),
                             ctx->indexing(0), symbolTable,
...
antlrcpp::Any DaphneDSLVisitor::visitIfStatement(
    ...
    for (auto it = owUnion.begin(); it != owUnion.end(); it++) {
        ...
        if (!CompilerUtils::equalUnknownAware(tyThen, tyElse)) {
            ...
            // error message).
            // TODO The variable name may be ambiguous (two vars mapped to the
            // same value).
            std::stringstream s;
...
DaphneDSLVisitor::visitCastExpr(DaphneDSLGrammarParser::CastExprContext *ctx) {
    ...
    if (ctx->DATA_TYPE()) {
        ...
        if (dtStr == "matrix") {
            ...
            // {utils.getValueTypeByName(ctx->VALUE_TYPE()->getText())};
                ...
                mlir::Type argType =
                    utils.valueOrError(visit(ctx->expr())).getType();
    ...
    return static_cast<mlir::Value>(builder.create<mlir::daphne::CastOp>(
        utils.getLoc(ctx->start), resType,
...
antlrcpp::Any DaphneDSLVisitor::visitRightIdxFilterExpr(
    ...
    if (ctx->cols) // cols specified
        obj = builder.create<mlir::daphne::FilterColOp>(
            ...
            obj, utils.valueOrError(visit(ctx->cols)));
...
antlrcpp::Any DaphneDSLVisitor::visitMatrixLiteralExpr(
    ...
    if (!ctx->literal().size())
        throw ErrorHandler::compilerError(
            utils.getLoc(ctx->start), "DSLVisitor",
            "can't infer type from an empty matrix");

    mlir::Location loc = utils.getLoc(ctx->start);
    mlir::Value currentValue =
        utils.valueOrError(visitLiteral(ctx->literal(0)));
    mlir::Value result;
    ...
    if (currentValue.getType().isSignedInteger(64)) {
        std::shared_ptr<int64_t[]> vals =
            std::shared_ptr<int64_t[]>(new int64_t[ctx->literal().size()]);
        for (unsigned i = 0; i < ctx->literal().size(); i++) {
            ...
            if (currentValue.getType() != valueType)
                throw ErrorHandler::compilerError(
                    utils.getLoc(ctx->start), "DSLVisitor",
                    "matrix of elements of different types");
            vals.get()[i] = CompilerUtils::constantOrThrow<int64_t>(
                currentValue,
                "elements of matrix literals must be parse-time constants");
        }
        auto mat = DataObjectFactory::create<DenseMatrix<int64_t>>(
            ctx->literal().size(), 1, vals);
        result = static_cast<mlir::Value>(
            builder.create<mlir::daphne::MatrixConstantOp>(
                loc, utils.matrixOf(valueType),
                builder.create<mlir::daphne::ConstantOp>(
                    loc, reinterpret_cast<uint64_t>(mat))));
    } else if (currentValue.getType().isF64()) {
        std::shared_ptr<double[]> vals =
            std::shared_ptr<double[]>(new double[ctx->literal().size()]);
        for (unsigned i = 0; i < ctx->literal().size(); i++) {
            currentValue = utils.valueOrError(visitLiteral(ctx->literal(i)));
            if (currentValue.getType() != valueType)
                throw ErrorHandler::compilerError(
        ...
        auto mat = DataObjectFactory::create<DenseMatrix<double>>(
            ctx->literal().size(), 1, vals);
        result = static_cast<mlir::Value>(
            builder.create<mlir::daphne::MatrixConstantOp>(
                loc, utils.matrixOf(valueType),
        ...
        std::shared_ptr<bool[]> vals =
            std::shared_ptr<bool[]>(new bool[ctx->literal().size()]);
        for (unsigned i = 0; i < ctx->literal().size(); i++) {
            currentValue = utils.valueOrError(visitLiteral(ctx->literal(i)));
            if (currentValue.getType() != valueType)
                throw ErrorHandler::compilerError(
        ...
        auto mat = DataObjectFactory::create<DenseMatrix<bool>>(
            ctx->literal().size(), 1, vals);
        result = static_cast<mlir::Value>(
            builder.create<mlir::daphne::MatrixConstantOp>(
                loc, utils.matrixOf(valueType),
        ...
        throw ErrorHandler::compilerError(
            utils.getLoc(ctx->start), "DSLVisitor",
            "invalid value type for matrix literal");
...
DaphneDSLVisitor::visitRange(DaphneDSLGrammarParser::RangeContext *ctx) {
    if (ctx->pos)
        return std::make_pair(
            true, antlrcpp::Any(utils.valueOrError(visit(ctx->pos))));
    else {
        mlir::Value posLowerIncl =
            ctx->posLowerIncl ? utils.valueOrError(visit(ctx->posLowerIncl))
                              : nullptr;
        mlir::Value posUpperExcl =
            ctx->posUpperExcl ? utils.valueOrError(visit(ctx->posUpperExcl))
                              : nullptr;
        return std::make_pair(
...
DaphneDSLVisitor::visitLiteral(DaphneDSLGrammarParser::LiteralContext *ctx) {
    ...
    if (auto lit = ctx->INT_LITERAL()) {
        ...
        //        if(litStr.length() > 2) {
        //            spdlog::debug("litstr: {} len: {}", litStr,
        //            litStr.length()); spdlog::debug("stringview: {}", ss);
        //            spdlog::debug("substringview: {}",
        //            ss.substr(litStr.length() - 3));
        //        }
        if (litStr.back() == 'u')
            return static_cast<mlir::Value>(
                builder.create<mlir::daphne::ConstantOp>(loc,
                                                         std::stoul(litStr)));
        else if ((ss.length() > 2) && std::string_view(litStr).substr(
                                          litStr.length() - 3) == "ull") {
