...
std::vector<void *> WorkerImpl::createPackedCInterfaceInputsOutputs(
    ...
    assert(static_cast<size_t>(functionType.getNumInputs()) ==
               workInputs.size() &&
           "Number of inputs received have to match number of MLIR fragment "
           "inputs");
...
Structure *WorkerImpl::readOrGetMatrix(const std::string &identifier,
    ...
    if (data_it != localData_.end()) {
        ...
        assert(localData_.insert({identifier, m}).second &&
               "Value should always be inserted");
        return m;
    }
}
