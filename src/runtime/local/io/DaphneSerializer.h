...
template <typename VT> struct DaphneSerializer<DenseMatrix<VT>, false> {
    ...
    deserializeHeader(const char *buf, DenseMatrix<VT> *matrix = nullptr) {
        size_t bufIdx = 0;
        assert((DF_Dtype(buf) == DF_data_t::DenseMatrix_t) &&
               "DenseMatrix deserialize(): DT mismatch");
        assert((DF_Vtype(buf) == ValueTypeUtils::codeFor<VT>) &&
               "DenseMatrix deserialize(): VT mismatch");
        // FF to the body
...
template <typename VT> struct DaphneSerializer<CSRMatrix<VT>, false> {
    ...
    static CSRMatrix<VT> *deserializeHeader(const char *buffer,
        ...
        assert((DF_Dtype(buffer) == DF_data_t::CSRMatrix_t) &&
               "CSRMatrix deserialize(): DT mismatch");
        assert((DF_Vtype(buffer) == ValueTypeUtils::codeFor<VT>) &&
               "CSRMatrix deserialize(): VT mismatch");
