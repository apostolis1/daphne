...
template <typename ValueType> class CSRMatrix : public Matrix<ValueType> {
    ...
    CSRMatrix(const CSRMatrix<ValueType> *src, size_t rowLowerIncl,
        ...
        assert((rowLowerIncl < src->numRows) &&
               "rowLowerIncl is out of bounds");
        assert((rowUpperExcl <= src->numRows) &&
               "rowUpperExcl is out of bounds");
        assert((rowLowerIncl < rowUpperExcl) &&
               "rowLowerIncl must be lower than rowUpperExcl");
  ...
  public:
    ...
    void shrinkNumNonZeros(size_t numNonZeros) {
        assert((numNonZeros <= getNumNonZeros()) &&
               "numNonZeros can only be shrinked");
