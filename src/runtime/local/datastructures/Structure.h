...
class Structure {
  ...
  protected:
    ...
    void clone_mdo(const Structure *src) {
        // FIXME: This clones the meta data to avoid locking (thread
        // synchronization for data copy)
        for (int i = 0; i < static_cast<int>(ALLOCATION_TYPE::NUM_ALLOC_TYPES);
             i++) {
            auto placements = src->mdo->getDataPlacementByType(
                static_cast<ALLOCATION_TYPE>(i));
  ...
  public:
    ...
    [[nodiscard]] size_t getNumItems() const { return numRows * numCols; }
