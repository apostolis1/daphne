...
class Frame : public Structure {
    ...
    bool tryValueType(Structure *colMat, ValueTypeCode *schemaSlot,
        ...
        if (auto colMat2 = dynamic_cast<DenseMatrix<VT> *>(colMat)) {
            assert((colMat2->getRowSkip() == 1) &&
                   "all given matrices must not be a view on a column of a "
                   "larger matrix");
  ...
  public:
    ...
    DenseMatrix<ValueType> *getColumn(size_t idx) {
        assert((ValueTypeUtils::codeFor<ValueType> == schema[idx]) &&
               "requested value type must match the type of the column");
