...
// this include file is placed here to solve a compilation issue with spdlog and
// catch2
...
template <class DT> bool operator==(const DT &lhs, const DT &rhs) {
    ...
    static bool apply(const Frame *lhs, const Frame *rhs, DCTX(ctx)) {
        ...
        for (size_t c = 0; c < numCols; c++) {
            ...
            case ValueTypeCode::F64:
                if (!checkEq(lhs->getColumn<double>(c),
                             rhs->getColumn<double>(c), ctx))
                    return false;
                break;
            case ValueTypeCode::F32:
                if (!checkEq(lhs->getColumn<float>(c), rhs->getColumn<float>(c),
                             ctx))
                    return false;
                break;
            case ValueTypeCode::SI64:
                if (!checkEq(lhs->getColumn<int64_t>(c),
                             rhs->getColumn<int64_t>(c), ctx))
                    return false;
                break;
            case ValueTypeCode::SI32:
                if (!checkEq(lhs->getColumn<int32_t>(c),
                             rhs->getColumn<int32_t>(c), ctx))
                    return false;
                break;
            case ValueTypeCode::SI8:
                if (!checkEq(lhs->getColumn<int8_t>(c),
                             rhs->getColumn<int8_t>(c), ctx))
                    return false;
                break;
            case ValueTypeCode::UI64:
                if (!checkEq(lhs->getColumn<uint64_t>(c),
                             rhs->getColumn<uint64_t>(c), ctx))
                    return false;
                break;
            case ValueTypeCode::UI32:
                if (!checkEq(lhs->getColumn<uint32_t>(c),
                             rhs->getColumn<uint32_t>(c), ctx))
                    return false;
                break;
            case ValueTypeCode::UI8:
                if (!checkEq(lhs->getColumn<uint8_t>(c),
                             rhs->getColumn<uint8_t>(c), ctx))
                    return false;
                break;
            default:
