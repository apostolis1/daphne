...
void columnIDSort(DenseMatrix<VTIdx> *&idx, const DenseMatrix<VT> *arg,
    ...
    for (const auto &group : groups)
        std::stable_sort(indicies + group.first, indicies + group.second,
                         compare);
}
...
void columnGroupScan(std::vector<std::pair<VTIdx, VTIdx>> &groups,
                     DenseMatrix<VT> *col, size_t colIdx, DCTX(ctx)) {
...
template <typename VTIdx, typename VT>
void multiColumnIDSort(DenseMatrix<VTIdx> *&idx, const DenseMatrix<VT> *col,
                       size_t colIdx,
...
struct Order<DenseMatrix<VTRes>, DenseMatrix<VTArg>> {
    ...
    apply(DenseMatrix<VTRes> *&res, const DenseMatrix<VTArg> *arg,
        ...
        if (returnIdx) {
            ...
            extractRow<DenseMatrix<VTArg>, DenseMatrix<VTArg>, size_t>(
                (DenseMatrix<VTArg> *&)res, arg, idx, ctx);
            DataObjectFactory::destroy(idx);
        }
    }
};
