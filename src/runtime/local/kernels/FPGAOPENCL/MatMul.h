...
#define KKK_gemv 1
        ...
        // #ifndef NDEBUG
        //	assert((nr1%(II*III)==0) && "lhs #rows number must be a multiple
        //of 448"); 	assert((nc1%(JJ*JJJ)==0 && nc1>512 && nr2%(JJ*JJJ)==0 &&
        //nc1>512) && "#cols of lhs and #rows of rhs must be a multiple of 512
        //(and minimum 1024)"); 	assert((nc2%(KK*KKK)==0) && "#cols of rhs must
        //be a multiple of 512"); #endif
...
#ifndef NDEBUG
        const size_t nr2 = rhs->getNumRows();
        assert((nc1 == nr2) && "#cols of lhs and #rows of rhs must be the
same"); #endif if(res == nullptr) res =
