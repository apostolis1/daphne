  ...
  public:
    ...
    LogicalResult matchAndRewrite(Operation *op,
        ...
        for (size_t i = 0; i < resultTypes.size(); i++)
            callee << "__"
                   << CompilerUtils::mlirTypeToCppTypeName(resultTypes[i],
                                                           false);
        ...
        if (
            ...
            for (size_t i = 0; i < numODSOperands; i++) {
                ...
                callee << "__"
                       << CompilerUtils::mlirTypeToCppTypeName(
                              odsOperandTy, generalizeInputTypes);
            ...
            for (size_t i = 0; i < operandTypes.size(); i++) {
                callee << "__"
                       << CompilerUtils::mlirTypeToCppTypeName(
                              operandTypes[i], generalizeInputTypes);
        ...
        if (auto distCompOp =
            ...
            callee << "__"
                   << CompilerUtils::mlirTypeToCppTypeName(strTy, false);
        ...
        auto kernel = rewriter.create<daphne::CallKernelOp>(
            loc, callee.str(), newOperands, op->getResultTypes());
...
class DistributedPipelineKernelReplacement
  ...
  public:
    ...
    DistributedPipelineKernelReplacement(MLIRContext *mctx, Value dctx,
