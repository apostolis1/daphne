 ...
 *
 * At the moment, this pass is implemented in a very simple way. It merely
 * harmonizes the value types of all inputs with those of the single output of
 * certain operations. This is because so far we mainly pre-compile our kernels
 * for homogeneous combinations of input/output types. The operations are marked
 * by traits.
 *
...
void AdaptTypesToKernelsPass::runOnOperation() {
    ...
    f.getBody().front().walk([&](Operation *op) {
        ...
        if (!operandIdxs.empty()) {
            ...
            // TODO Support this.
            // Skip pure frame ops, since we cannot easily cast the column types
            // of frames anyway.
