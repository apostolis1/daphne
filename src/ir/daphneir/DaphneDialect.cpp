...
template <class AttrElementT,
          ...
          class CalculationT = std::function<
              ElementValueT(const ElementValueT &, const ElementValueT &)>>
mlir::Attribute constFoldBinaryOp(mlir::Type resultType,
...
template <class AttrElementT,
          class ElementValueT = typename AttrElementT::ValueType,
          class CalculationT =
              std::function<bool(const ElementValueT &, const ElementValueT &)>>
mlir::Attribute constFoldBinaryCmpOp(llvm::ArrayRef<mlir::Attribute> operands,
    ...
    if (llvm::isa<AttrElementT>(operands[0]) &&
        ...
        auto lhs = operands[0].cast<AttrElementT>();
        auto rhs = operands[1].cast<AttrElementT>();
        return mlir::BoolAttr::get(lhs.getContext(),
                                   calculate(lhs.getValue(), rhs.getValue()));
    }
    return {};
}
...
mlir::OpFoldResult mlir::daphne::CastOp::fold(FoldAdaptor adaptor) {
    ...
    if (auto in = operands[0].dyn_cast_or_null<IntegerAttr>()) {
        ...
        if (getType().isF64()) {
            ...
            if (in.getType().isUnsignedInteger() || in.getType().isIndex()) {
                return FloatAttr::getChecked(
                    getLoc(), getType(),
                    llvm::APIntOps::RoundAPIntToDouble(in.getValue()));
            }
        }
        if (getType().isF32()) {
            if (in.getType().isSignedInteger()) {
            ...
            if (in.getType().isUnsignedInteger()) {
                return FloatAttr::get(
                    getType(),
                    llvm::APIntOps::RoundAPIntToFloat(in.getValue()));
            }
        }
    }
    if (auto in = operands[0].dyn_cast_or_null<FloatAttr>()) {
        ...
        if (getType().isF32()) {
            return FloatAttr::getChecked(getLoc(), getType(),
...
mlir::OpFoldResult mlir::daphne::EwDivOp::fold(FoldAdaptor adaptor) {
    ...
    if (getType().isSignedInteger()) {
        if (auto res =
                constFoldBinaryOp<IntegerAttr>(getType(), operands, sintOp))
            return res;
    } else if (getType().isUnsignedInteger()) {
        if (auto res =
                constFoldBinaryOp<IntegerAttr>(getType(), operands, uintOp))
            return res;
    }
    return {};
}
...
mlir::LogicalResult
mlir::daphne::EwEqOp::canonicalize(mlir::daphne::EwEqOp op,
                                   PatternRewriter &rewriter) {
    mlir::Value lhs = op.getLhs();
    mlir::Value rhs = op.getRhs();
...
mlir::daphne::EwAddOp::canonicalize(mlir::daphne::EwAddOp op,
    ...
    if (lhsIsStr || rhsIsStr) {
        ...
        rewriter.replaceOpWithNewOp<mlir::daphne::ConcatOp>(op, strTy, lhs,
                                                            rhs);
        return mlir::success();
    } else {
        const bool lhsIsSca =
