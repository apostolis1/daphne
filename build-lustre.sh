...
# This particular version is used to build only the docker images with lustre support
# It is a slight modification of the build.sh script on commit 6da3edf4903eccae2e8de90f8c1d4ea617a898b9
# After the images are built, you can build the actual daphne executable using the build.sh script from the commit you want (been tested only with commit
# 336420e4b40e20a6ab43709d78c95edc61b231a7 ) 
...
#******************************************************************************
source software-package-versions-lustre.txt
...
if [ $WITH_DEPS -gt 0 ]; then
    ...
    # cd ${sourcePrefix}
    # mkdir -p ${sourcePrefix}/lustre_thirdparty
    # Temp workaround (cloning on / folder) because the previous path wasn't working
    # TODO: Investigate whether the path matters or if there was another issue preventing previous builds
    cd /
    git clone https://github.com/lustre/lustre-release --depth=1 --branch=b2_15
    daphne_msg "Success downloading lustre"
    daphne_msg "Building lustre"
    cd lustre-release
    sh autogen.sh
    ./configure --disable-server --disable-modules --prefix=$installPrefix \
    CPPFLAGS="-I/lustre-release/lnet/include/uapi -I/lustre-release/lustre/include/uapi"
    make
    make install
    daphne_msg "Success building lustre"
    # This needs to be preserved by hand
    # By default `make install` on lustre puts the contents of the linux dir on /usr/include/linux/
    # Another solution would be to preserve the whole /usr/include/linux directory below with a docker COPY
    cp -r lustre/include/uapi/linux $installPrefix/include
    cd - > /dev/null
